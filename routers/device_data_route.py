...
# from database import device_data, users_data  # MongoDB collections
...
# templates = Jinja2Templates(directory='templates')  # Setup Jinja2 template directory
...
#     - Renders device-data.html template with all IoT device records.
#     """
#     try:
#         # Step 1: Extract current user's email from token
#         user_email = current_user["email"]

#         # Step 2: Look up user from database
#         user = users_data.find_one({'email': user_email})
#         logger.info(f"Device data page accessed by user: {user_email}")

#         # Step 3: If user not found, raise 401 Unauthorized
#         if not user:
#             logger.warning(f"User not found: {user_email}")
#             raise HTTPException(
...
#         # Step 5: If user is not admin, show 404 error page
#         if role != "admin":
#             return templates.TemplateResponse('error-404.html', {'request': request})

#         # Step 6: Fetch all device data from the database
#         devices = list(device_data.find())
#         logger.info(f"Fetched devices: {devices}")
#         logger.info(f"Rendering device data page for user: {username}, role: {role}")

#         # Step 7: Render the device-data.html template with the context
#         return templates.TemplateResponse(
#             'device-data.html',
#             {
#                 'request': request,
#                 "role": role,
#                 'username': username,
#                 'devices': devices  # Pass device records to frontend
#             }
#         )

#     # Specific HTTP errors are re-raised unchanged
#     except HTTPException:
#         raise
